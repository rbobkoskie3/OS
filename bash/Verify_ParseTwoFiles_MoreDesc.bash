...
# ID: rb868x 12629 Dec  9 21:24 Verify_ParseTwoFiles_MoreDesc.bash
...
c=`echo $a $b 100 | awk '{ print $1/$2*$3 }'`      # Use awk for in division. SESSION_ID should never be 0, e.g., there will always be SESSION_ID's in the first data file
...
if [[ -s $SESSION_ID ]]; then
   echo "Removing the first $OFFSET fields from data files ..."
...
for i in $FIELDS_FOR_COMPARE; do
   ...
   cat $DATA_INPUT_1 |grep "^##Desc\[0\]" |cut -d',' -f$(($i+$OFFSET-$OFFSET_COMPARE-$OFFSET_NARUS)) | awk -F'\r' '{ printf "%-25s %-25s", $1, "" }' >> $DATA_DIFF
   ...
   diff -U 0 $COMPARE_01 $COMPARE_02 |egrep -v ^'@|\-|\+\+\+|No difference' |wc -l | awk '{ printf "%-11s", $1 }' >> $DATA_DIFF
...
for i in $FIELDS_FOR_COMPARE; do
   ...
   # Add data to 2nd and 3rd columns of output file, e.g., the data for DESC[0] DESC[1] respectively
   cat $DATA_INPUT_1 |grep "^##Desc\[1\]" |cut -d',' -f$(($i+$OFFSET-$OFFSET_COMPARE-$OFFSET_NARUS)) | awk -F'\r' '{ printf "%-25s %-25s", "", $1 }' >> $DATA_DIFF

   # Obtain diffs, e.g., Add data to 4th column of output file, e.g., the data for #DIFFS
   # NOTE that the egrep '\-|\+\+\+' looks for a single '-', this will effectively count the
   # diffs, e.g., each diff WILL NOT be counted twice
   diff -U 0 $COMPARE_11 $COMPARE_12 |egrep -v ^'@|\-|\+\+\+|No difference' |wc -l | awk '{ printf "%-11s", $1 }' >> $DATA_DIFF
