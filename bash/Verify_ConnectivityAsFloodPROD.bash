...
# ID: 10060 Jan 26 17:00 Verify_ConnectivityAsFlood.bash
...
if [ ! -w ~flood ]; then
   echo "\"$USER\" cannot execute this command...must be flood.  Exiting..."
   exit 1
fi
...
if [ $FQDN ]; then
   ...
   if grep 'Connect.* to' $VERIFY_SOCKET 1>/dev/null 2>&1; then
      ...
      ssh -i ~flood/.ssh/batch_remote floodxfr@$FQDN hostname > $VERIFY_CONNECTION
      if [ $? -ne 0 ]; then
   ...
   if grep 'Connect.* to' $VERIFY_SOCKET 1>/dev/null 2>&1; then
      ...
      floodenv python-flood Verify_Connectivity_DBC_9017.py > $VERIFY_CONNECTION
...
if [ $FQDN ]; then
   ...
   if grep 'Connect.* to' $VERIFY_SOCKET 1>/dev/null 2>&1; then
      ...
      floodenv python-flood Verify_Connectivity_IGW_9017.py > $VERIFY_CONNECTION
      grep -i error $VERIFY_CONNECTION 1>/dev/null 2>&1
      if [ $? -eq 0 ]; then
...
if [ $FQDN ]; then
   ...
   if grep 'Connect.* to' $VERIFY_SOCKET 1>/dev/null 2>&1; then
      ...
      ssh -p $PORT -i ~flood/.ssh/fathom_srv_grp_dsa fathom@$FQDN hostname > $VERIFY_CONNECTION
      if [ $? -ne 0 ]; then
...
if [ $FQDN ]; then
   ...
   if grep 'Connect.* to' $VERIFY_SOCKET 1>/dev/null 2>&1; then
      ...
      ssh -i ~flood/.ssh/batch_remote floodxfr@flanlport200 hostname > $VERIFY_CONNECTION
      if [ $? -ne 0 ]; then
