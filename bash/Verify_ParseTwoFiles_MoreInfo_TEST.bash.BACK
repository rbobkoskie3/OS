...
if [ $c -lt $PERCENT ]; then
   ...
   rm /tmp/DESC_* 2>/dev/null
   rm /tmp/VERIFY_* 2>/dev/null
   rm /tmp/COMPARE_* 2>/dev/null
...
function createFileDesc() {
   ...
   desc=`ls /tmp/DESC_$numdata$numvector-$TIMESTAMP`

   grep "^\[$numvector\]" $DATA_1 > $desc
...
function createFileVerify() {
   ...
   desc=`ls /tmp/DESC_$numdata$numvector-$TIMESTAMP`
   verify=`ls /tmp/VERIFY_$numdata$numvector-$TIMESTAMP`
...
function findDiffs() {
   ...
   verify1=`ls /tmp/VERIFY_$inputdata1$numvector-$TIMESTAMP`
   verify2=`ls /tmp/VERIFY_$inputdata2$numvector-$TIMESTAMP`
   compare1=`ls /tmp/COMPARE_$inputdata1$numvector-$TIMESTAMP`
   compare2=`ls /tmp/COMPARE_$inputdata2$numvector-$TIMESTAMP`
   ...
   cat $DATA_INPUT_1 |grep "^##Desc\[$numvector\]" |cut -d',' -f$(($field+$offset-$offsetcompare-$offsetnarus)) | awk -F'\r' '{ printf "%-25s", $1 }' >> $SORT
...
# echo -e "FIELD Desc[*] #DIFFS #COMPARE" | awk '{ printf "\n%-10s %-26s %-10s %-10s\n\n", $1, $2, $3, $4 }' >> $DATA_DIFF
